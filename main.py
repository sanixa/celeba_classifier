...
def main():
    ...
    parser.add_argument('--test-batchsize', type=int, default=256)
    ...
    os.system('touch ' + os.path.join(save_dir, 'seed=_%s' % str(args.random_seed)))
    ...
    trainset = CelebA(root=os.path.join('../GS-WGAN-custom/data'), split='train',
        transform=transform_train, download=False, custom_subset=True)
    testset = CelebA(root=os.path.join('/work/u5366584/exp', 'datasets', 'celeba'), split='test',
        transform=transform_train, download=False)
    
    workers = 0
    dataloaders_dict = {}
    dataloaders_dict['train'] = torch.utils.data.DataLoader(trainset, batch_size=args.batchsize,
                                             shuffle=True, num_workers=workers)
    dataloaders_dict['val'] = torch.utils.data.DataLoader(testset, batch_size=args.test_batchsize,
                                         shuffle=True, num_workers=workers)
    ...
    torch.save(model_ft.state_dict(), os.path.join(save_dir, args.exp_name))
